    ...
    <header className="sticky top-0 z-40 border-b border-border bg-background/90 backdrop-blur">
      <div className="container mx-auto max-w-6xl flex h-14 items-center justify-between px-4">
        <Link href="/" className="flex items-center gap-2 font-semibold">
          <BookOpen className="h-5 w-5" /> {wikiConfig.siteName}
        </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          <a href={wikiConfig.githubUrl} target="_blank" rel="noreferrer" className="text-sm hover:underline hidden md:inline">GitHub</a>
      ...
      <Sheet open={open} onClose={() => setOpen(false)}>
        <nav className="flex flex-col gap-4 mt-8 text-base">
          ...
          <a href={wikiConfig.githubUrl} target="_blank" rel="noreferrer" className="hover:underline">GitHub</a>
        </nav>
      </Sheet>
    </header>
  )
